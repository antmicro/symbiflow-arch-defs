...
def create_get_switch(conn, get_pip_timing=None):
...
WHERE
  ...
  AND site_type_pkey = ?""", (site_pin.name, site_types[site.type])
            ...
            if site_pin.timing is not None:
                ...
                drive_resistance, capacitance, intrinsic_delay = \
                    get_site_pin_timing(site_pin)
...
def classify_nodes(conn, get_switch_timing, check_pip_for_direct=None):
    write_cur = conn.cursor()
...
""", (node, )
            ...
            force_direct = False
            if check_pip_for_direct is not None:
                ...
                force_direct = check_pip_for_direct(pip_name)
...
def import_segments(conn, db, segment_wire_map):
    ...
    for extra_segment in [segment_wire_map.default_segment, VCC_NET, GND_NET]:
        if extra_segment not in segment_wire_map.get_segments():
            write_cur.execute(
                """
INSERT INTO segment(name) VALUES (?)
    ...
    for segment in segment_wire_map.get_segments():
        write_cur.execute(
            """
INSERT INTO segment(name) VALUES (?)
            """, (segment, )
