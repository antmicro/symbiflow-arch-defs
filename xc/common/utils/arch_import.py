...
from tile_import import add_vpr_tile_prefix
...
def is_in_roi(conn, roi, tile_pkey):
    ...
    # For the code to work both for X-Ray and U-Ray the check whether a tile
    # belongs to the Roi or a grid limit had to be moved here.
    #
    # This is because this code is generic and therefore cannot instance
    # GridLoc objcets which are necessary for a call to Roi.tile_in_roi().

    # ROI given as a tuple (xmin, ymin, xmax, ymax)
    if isinstance(roi, tuple):
        for loc in phy_locs:
            if roi[0] <= loc[0] and loc[0] <= roi[2] and \
               roi[1] <= loc[1] and loc[1] <= roi[3]:
                return True

        return False

    # ROI given as a Roi object (X-Ray only!)
    else:
        for loc in phy_locs:
            if roi.x1 <= loc[0] and loc[0] <= roi.x2 and \
               roi.y1 <= loc[1] and loc[1] <= roi.y2:
                return True

        return False


def make_prefix(site, x, y, site_prefixes, from_site_name=False):
    ...
    prefix_required = site_prefixes[site_type]
...
def get_site_prefixes(conn, tile_pkey, site_prefixes):
    cur = conn.cursor()
...
AND
  ...
  """, (tile_pkey, tile_pkey, tile_pkey)
    ...
    for site_name, x, y in cur:
        site_type, prefix = make_prefix(site_name, x, y, site_prefixes, from_site_name=True)
        assert site_type not in prefixes
        prefixes[site_type] = prefix

    return prefixes


def create_capacity_prefix(c, tile_prefix, tile_pkey, tile_capacity, site_prefixes):
...
ORDER BY site_instance.x_coord, site_instance.y_coord, site_type.name;""",
    ...
    for site_type, x, y in c:
        _, prefix = make_prefix(site_type, x, y, site_prefixes)
...
def get_fasm_tile_prefix(conn, g, tile_pkey, site_as_tile_pkey, tile_capacity, site_prefixes):
...
WHERE
    ...
    if len(tile_type_map) > 1:
        ...
        tile_type_map.update(get_site_prefixes(conn, tile_pkey, site_prefixes))
        ...
        if tile_capacity > 1:
            tile_prefix = create_capacity_prefix(
                c, tile_prefix, tile_pkey, tile_capacity, site_prefixes
            )
...
def get_tiles(
        conn, g, roi, synth_loc_map, synth_tile_map, tile_types, tile_capacity, site_prefixes
):
    ...
    for tile_pkey, grid_x, grid_y, phy_tile_pkey, tile_type_pkey, site_as_tile_pkey in c.execute(
        ...
        if tile_type.startswith('PSS'):
            ...
            meta_fun = get_fasm_tile_prefix(
                conn, g, tile_pkey, site_as_tile_pkey, tile_capacity[tile_type], site_prefixes
            )
...
def arch_import(args, db, roi, use_roi, use_overlay, site_prefixes):
